...
function calValidDistance(distance, chartData, config, opts) {
    ...
    if (distance >= 0) {
        ...
        if (Math.abs(distance) >= dataChartWidth - dataChartAreaWidth) {
...
function getSeriesDataItem(series, index) {
    ...
    for (let i = 0; i < series.length; i++) {
        ...
        if (item.data[index] !== null && typeof item.data[index] !== 'undefined' && item.show) {
            ...
            if (item.format) {
                seriesItem.data = item.format(item.data[index]);
            } else {
                seriesItem.data = item.data[index];
            }
...
function getCandleToolTipData(series, seriesData, calPoints, index, categories, extra) {
    ...
    let downColor = extra.color.downFill; //颜色顺序为开盘，收盘，最低，最高
...
function isInExactChartArea(currentPoints, opts, config) {
    return (
        currentPoints.x < opts.width - opts.area[1] + 10 && currentPoints.x > opts.area[3] - 10 && currentPoints.y > opts.area[0] && currentPoints.y < opts.height - opts.area[2]
    );
}
...
function getGaugeAxisPoints(categories, startAngle, endAngle) {
    ...
    for (let i = 0; i < categories.length; i++) {
        if (categories[i].value === null) {
            categories[i].value = 0;
        } else {
            categories[i].value = categories[i].value;
        }
...
function calYAxisData(series, opts, config) {
    ...
    var rangesFormat = ranges.map(function (item) {
        ...
        if (opts.yAxis.format) {
            item = opts.yAxis.format(Number(item));
        } else {
            item = item;
        }
...
function drawPieText(series, opts, config, context, radius, center) {
    ...
    for (let i = 0; i < seriesConvert.length; i++) {
        let item = seriesConvert[i]; // line end

        let orginX1 = Math.cos(item.arc) * (item.radius + lineRadius);
        let orginY1 = Math.sin(item.arc) * (item.radius + lineRadius); // line start

        let orginX2 = Math.cos(item.arc) * item.radius;
        let orginY2 = Math.sin(item.arc) * item.radius; // text start
...
function drawToolTipSplitLine(offsetX, opts, config, context) {
    ...
    if (toolTipOption.gridType == undefined) {
        toolTipOption.gridType = 'solid';
    } else {
        toolTipOption.gridType = toolTipOption.gridType;
    }

    if (toolTipOption.dashLength == undefined) {
        toolTipOption.dashLength = 4;
    } else {
        toolTipOption.dashLength = toolTipOption.dashLength;
    }
            ...
            case 'group':
                ...
                points.forEach(function (item, index) {
                    if (item !== null) {
                        ...
                        context.lineTo(startX, item.y); //context.rect(startX, item.y, item.width, height);
...
function drawCandleDataPoints(series, seriesMA, opts, config, context) {
    ...
    if (candleOption.average.show) {
        seriesMA.forEach(function (eachSeries, seriesIndex) {
            var data = eachSeries.data;
            var points = getDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, process); //calPoints.push(points);
...
function drawAreaDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        for (let i = 0; i < splitPointList.length; i++) {
            let points = splitPointList[i]; // 绘制区域数
...
function drawLineDataPoints(series, opts, config, context) {
    ...
    if (lineOption.type) {
        lineOption.type = lineOption.type;
    } else {
        lineOption.type = 'straight';
    }

    if (lineOption.width) {
        lineOption.width = lineOption.width;
    } else {
        lineOption.width = 2;
    }
...
function drawMixDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        if (eachSeries.type == 'area') {
            ...
            for (let i = 0; i < splitPointList.length; i++) {
                let points = splitPointList[i]; // 绘制区域数据
...
function drawXAxis(categories, opts, config, context) {
    ...
    if (opts.xAxis.disabled !== true) {
        // 对X轴列表做抽稀处理
        let validWidth = opts.width - opts.padding[1] - opts.padding[3] - config.yAxisWidth - config.yAxisTitleWidth; //默认全部显示X轴标签

        let maxXAxisListLength = categories.length; //如果设置了X轴单屏数量
...
function drawYAxisGrid(categories, opts, config, context) {
    ...
    let xAxiseachSpacing = opts.chartData.xAxisData.eachSpacing;
...
function drawLegend(series, opts, config, context, chartData) {
    ...
    let lineHeight = Math.max(opts.legend.lineHeight * opts.pixelRatio, fontSize); //画背景及边框
...
function drawArcbarDataPoints(series, opts, config, context) {
    ...
    radius -= arcbarOption.width / 2; //背景颜色
...
function drawRadarDataPoints(series, opts, config, context) {
    ...
    var radius = Math.min(centerPosition.x - (getMaxTextListLength(opts.categories) + config.radarLabelTextMargin), centerPosition.y - config.radarLabelTextMargin); //TODO逻辑不对

    radius -= opts.padding[1]; // draw grid
...
function Animation(opts) {
    ...
    var _step = function step(timestamp) {
        if (timestamp === null || this.isStop === true) {
            ...
            if (opts.onAnimationFinish) {
                opts.onAnimationFinish();
            }
...
function drawCharts(type, opts, config, context) {
    ...
    opts._series_ = series = filterSeries(series); //重新计算图表区域

    opts.area = new Array(4); //复位绘图区域
    ...
    var legendWidth = _calLegendData.area.wholeWidth; //TODO废弃config.legendHeight参数
    ...
    if (opts.categories && opts.categories.length) {
        ...
        let xAxisHeight = _calCategoriesData.xAxisHeight;
        let angle = _calCategoriesData.angle;
    ...
    if (opts.enableScroll && opts.xAxis.scrollAlign == 'right' && opts._scrollDistance_ === undefined) {
        let offsetLeft = 0;
        let xAxisPoints = opts.chartData.xAxisData.xAxisPoints;
        let startX = opts.chartData.xAxisData.startX;
        let endX = opts.chartData.xAxisData.endX;
    ...
    if (type === 'pie' || type === 'ring' || type === 'rose') {
        if (opts.dataLabel === false) {
            config._pieTextMaxLength_ = 0;
        } else {
            config._pieTextMaxLength_ = getPieTextMaxLength(seriesMA);
        }
    }
...
var Charts = function Charts(opts) {
    if (opts.pixelRatio) {
        opts.pixelRatio = opts.pixelRatio;
    } else {
        opts.pixelRatio = 1;
    }

    if (opts.fontSize) {
        opts.fontSize = opts.fontSize * opts.pixelRatio;
    } else {
        opts.fontSize = 13 * opts.pixelRatio;
    }
    ...
    if (opts.colors) {
        ...
        config$$1.colors = config$$1.colors;
    }

    if (opts.yAxis.disabled !== true && opts.yAxis.title) {
        config$$1.yAxisTitleWidth = config$$1.yAxisTitleWidth;
    } else {
        config$$1.yAxisTitleWidth = 0;
    }

    if (opts.type == 'pie' || opts.type == 'ring') {
        if (opts.dataLabel === false) {
            config$$1.pieChartLinePadding = 0;
        } else {
            config$$1.pieChartLinePadding = opts.extra.pie.labelWidth * opts.pixelRatio || config$$1.pieChartLinePadding * opts.pixelRatio;
        }
    }
    ...
    if (opts.type == 'rose') {
        if (opts.dataLabel === false) {
            ...
            config$$1.pieChartLinePadding = opts.extra.rose.labelWidth * opts.pixelRatio || config$$1.pieChartLinePadding * opts.pixelRatio;
        }
    }

    if (opts.dataLabel === false) {
        config$$1.pieChartTextPadding = 0;
    } else {
        config$$1.pieChartTextPadding = config$$1.pieChartTextPadding * opts.pixelRatio;
    }

    if (opts.yAxis.splitNumber) {
        config$$1.yAxisSplit = opts.yAxis.splitNumber;
    } else {
        config$$1.yAxisSplit = config.yAxisSplit;
    } //屏幕旋转
...
Charts.prototype.zoom = function () {
    ...
    this.opts.xAxis.itemCount = val.itemCount; //重新计算x轴偏移距离
